    ...
    \begin{tikzpicture}
        \begin{umlpackage}{GridGenerator}
            ...
            \umlclass[x=-2, y=0]{Grid}{
                ...
                + resetGrid() \\
                + serialize(filepath: std::string\&) const \\
                \umlstatic{+ deserialize(filepath: std::string\&): Grid}
            }
            ...
            \umlsimpleclass[x=10, y=2]{ObstacleGenerator}

            \umlsimpleclass[x=-8.1, y=10]{Cell}

            \umluniassoc[anchor1=67, mult2=2..*]{Grid}{GridCoordinate}
            \umlunicompo[anchor1=131, mult2=width$\cdot$height]{Grid}{Cell}
            \umlassoc[anchor1=-13]{Grid}{GridSolvedStatus}
            \umldep[anchor1=13]{Grid}{ObstacleGenerator}
        \end{umlpackage}
    \end{tikzpicture}
\end{document}