...
package body dijkstra is
   ...
   is
      current_coord           : Pathfinder_C_Inf.Grid_Coord :=
        (x => current_coord_x, y => current_coord_y);
      neighbors               : constant Neighbors_Array :=
        get_neighbors_from_coord (grid, current_coord.x, current_coord.y);
      current_costs           : constant Cell_Cost :=
        get_cell_cost (grid, current_coord.x, current_coord.y);
      current_g               : constant Pathfinder_C_Inf.Safe_Cost :=
        current_costs.gCost;
      neighbor_coord          : Grid_Coord;
      neighbor_costs          : Cell_Cost;
      neighbor_state          : Cell_State;
      neighbor_g_from_current : Pathfinder_C_Inf.C_Double;
   begin
